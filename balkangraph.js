...
var OrgChart = function(b, a) {
    ...
    if (a) {
        for (var c in this.config) {
            if (typeof a[c] === "object" && !Array.isArray(a[c])) {
                this.config[c] = BALKANGraph._ay(this.config[c], a[c])
            } else {
    ...
    if (!this.config.xScrollUI) {
        this.xScrollUI = new OrgChart.xScrollUI(this.element, this.config, function() {
            ...
            d._l(true, BALKANGraph.action.xScroll)
        })
    }
...
OrgChart.prototype._l = function(s, a, c, g) {
    ...
    if (a === BALKANGraph.action.init && r != null) {
        y = r
    }
    ...
    if (a === BALKANGraph.action.centerNode) {
        ...
        if (r && this.response && (r[0] != this.response.viewBox[0] || r[1] != this.response.viewBox[1] || r[2] != this.response.viewBox[2] || r[3] != this.response.viewBox[3]) && (a === BALKANGraph.action.insert || a === BALKANGraph.action.expandCollapse || a === BALKANGraph.action.update || a === BALKANGraph.action.init)) {
            BALKANGraph.anim(this.getSvg(), {
                viewbox: r
            }, {
                viewbox: this.response.viewBox
...
OrgChart.prototype.fit = function() {
    ...
    this._l(true, BALKANGraph.action.init)
};
...
OrgChart.ui = {
    ...
    node: function(c, e, b, d, u, v, k, a) {
        ...
        var l = r.node.replaceAll("{w}", c.w).replaceAll("{h}", c.h);
        if (r.defs) {
        ...
        if (c.isGroup) {
            ...
            if (i) {
                ...
                h = h.replaceAll("{randId}", BALKANGraph._aQ()).replaceAll("{randId2}", BALKANGraph._aQ());
                l += h
            }
            l += h
        } else {
        ...
        var n = BALKANGraph.nodeOpenTag.replace("{id}", c.id).replace("{class}", q).replace("{level}", c.level).replace("{x}", o.x).replace("{y}", o.y);
    ...
    secondLinks: function(m, l, c, b) {
        ...
        for (var f = 0; f < l.spids.length; f++) {
            ...
            p = p.join(" ");
            e += BALKANGraph.secondLinkOpenTag.replace("{id}", l.id).replace("{child-id}", u) + v.secondLink.replace("{d}", p);
...
BALKANGraph.toolbarUI.prototype.init = function(c, f) {
    this.obj = c;
    this.toolbar = f;
    if (f) {
        this._aJ = false;
        ...
        var d = this.div.querySelector('[data-tlbr="plus"]');
        var b = this.div.querySelector('[data-tlbr="minus"]');
        var a = this.div.querySelector('[data-tlbr="fit"]');
        var e = this;
        d.addEventListener("click", function() {
            e.obj.zoom(true, null, true)
        });
        b.addEventListener("click", function() {
            e.obj.zoom(false, null, true)
        });
        a.addEventListener("click", function() {
            e.obj.fit()
        });
        ...
        this.layouts.addEventListener("click", function(g) {
            var h = g.target;
            while (h) {
                if (h.hasAttribute("data-layout")) {
                    h = h.getAttribute("data-layout");
                    e.obj.setLayout(BALKANGraph[h]);
                    break
                }
                h = h.parentNode
            }
...
OrgChart.server.prototype.search = function(n) {
    ...
    for (var e = 0; e < this.config.nodes.length; e++) {
        ...
        for (var k in h) {
            ...
            if (l && l.toString().toLowerCase().indexOf(n) != -1 && !BALKANGraph._Z(this.config, k)) {
                d.push({
                    id: h.id,
                    propertyName: k,
                    isId: (k == BALKANGraph.ID),
                    isNodeParam: f(k),
                    weight: l.toString().toLowerCase().indexOf(n),
                    data: h
                })
            }
        }
...
BALKANGraph._q = function(a) {
    return '<a style="text-decoration: none;position:absolute;bottom:' + a + "px;left:" + a + 'px;" title="OrgChart | BALKANGraph" href="https://testBALKANGraph.com"><span style="color:#039BE5;font-family:Roboto,Roboto-Regular,Helvetica;font-weight:bold;">LIYANAGE</span><span style="color:#F57C00;font-family:Roboto,Roboto-Regular,Helvetica;">Graph</span></a>'
};
    ...
    var l = function(v) {
        if (d) {
            ...
            if (h != null) {
                var x = r.getBGNode(h);
                if (x.isChildOfGroup) {
                    r.getNodeElement(x._5).style.opacity = 1;
                    g.style.opacity = 1
                } else {
                    if (!x.isGroup) {
                        g.style.opacity = 1
            ...
            if (h != null) {
                var x = r.getBGNode(h);
                if (x.isChildOfGroup) {
                    r.getNodeElement(x._5).style.opacity = 0.8;
                    g.style.opacity = 0.5
                } else {
                    if (x.isGroup) {
                        h = null;
